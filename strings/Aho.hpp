...
struct Aho {
  ...
  vector<int> word_indices_by_depth; // List of word indices in decreasing order of depth
  vector<int> defer; // Stores the index of the word which ends at the same position as this.
  ...
  int add_word(const string& word, int word_index)
  {
    ...
    int curr = 0;
    for (char c : word)
      curr = get_or_add_child(curr,c);
    if (nodes[curr].word_index < 0)
      nodes[curr].word_index = word_index;
  ...
  int get_suffix_link(int location, char c) const {
    if (location >= 0)
      location = nodes[location].link[c-'a'];
