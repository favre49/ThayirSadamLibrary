...
//
// Semiring where + = min, *= max. Semirings can be used in matrix exponentiation
template <class T>
struct min_max_semiring {
  ...
  min_max_semiring& operator+=(const min_max_semiring& o) { return *this = *this + o; }
  min_max_semiring operator*(const min_max_semiring& o) const {
    return (this->val > o.val ? this->val : o.val);
  }
  min_max_semiring& operator*=(const min_max_semiring& o) { return *this = *this * o; }
  friend ostream& operator<<(ostream& os, const min_max_semiring &x) { return os << x.val; }
};
