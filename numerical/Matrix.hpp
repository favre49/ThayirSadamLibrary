...
// Source: Lain
// Tested on: https://judge.yosupo.jp/problem/matrix_det,
...
struct Matrix {
 public:
  ...
  Matrix& operator*=(const Matrix& B) {
    ...
    for (int i = 0; i < n; i++)
      for (int k = 0; k < m; k++)
        for (int j = 0; j < bm; j++) C[i][j] += (*this)[i][k] * B[k][j];
    A.swap(C);
    return *this;
  }
  ...
  friend vector<T> operator*(const Matrix& A, const vector<T>& v) {
    ...
    vector<T> ret(n);
    for (int i = 0; i < n; i++)
      for (int j = 0; j < m; j++) ret[i] += A[i][j] * v[j];
    return ret;
  }
  ...
  friend vector<T> operator*(const vector<T>& v, const Matrix& A) {
    ...
    vector<T> ret(m);
    for (int i = 0; i < n; i++)
      for (int j = 0; j < m; j++) ret[j] += A[i][j] * v[j];
    return ret;
  }
  ...
  int inverse() {
    ...
    for (int i = 0; i < n; i++) {
      ...
      T inv = T(1) / tmp[i][i];
      for (int j = 0; j < m; j++) ret[i][j] *= inv;
      for (int j = i + 1; j < m; j++) tmp[i][j] *= inv;
      for (int h = 0; h < n; h++) {
        if (i == h) continue;
        const T c = -tmp[h][i];
        for (int j = 0; j < m; j++) ret[h][j] += ret[i][j] * c;
        for (int j = i + 1; j < m; j++) tmp[h][j] += tmp[i][j] * c;
      }
    }
    swap(A, ret.A);
    return rank;
  ...
  T determinant() {
    ...
    for (int i = 0; i < n; i++) {
      ...
      for (int j = i + 1; j < n; j++) {
        T a = B[j][i];
        if (a == 0) continue;
        for (int k = i; k < m; k++) B[j][k] -= B[i][k] * a;
      }
    }
    return prod;
  }
  ...
  // Choose largest pivot in the case of R, any non-zero pivot in the case of
  // Z_p
