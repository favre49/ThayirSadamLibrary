...
// Builds in O(nmlognlogm), O(1) queries
template<typename T>
struct RMQ2D {
  int n,m;
  tensor<T,4> table;
  ...
  int query(int r1, int c1, int r2, int c2) {
    ...
    int a = logs[(r2-r1)+1];
    int b = logs[(c2-c1)+1];
    return max({table[{r1,c1,a,b}],table[{r2-(1<<a)+1,c1,a,b}],table[{r1,c2-(1<<b)+1,a,b}], table[{r2-(1<<a)+1,c2-(1<<b)+1,a,b}]});
  }
};
